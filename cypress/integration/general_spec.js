...
describe('General Spec', () => {
	...
	context('PBC-128, PBC-653, PBC-722: Feedback modal window feature', () => {
		...
		it('PBC-128: Feedback modal window feature', () => { // PBC-128
			...
			cy.get('div.modal__title').should('contain', i18n._Feedback_);
			cy.get('div.modal__body > form > span').should('contain', i18n._HowWouldYouRateYourExp_);
			cy.getByAutoId('thumbUpBtn').should('exist');
			cy.getByAutoId('thumbDownBtn').should('exist');
			cy.getByAutoId('closeModalIcon').should('exist');
			cy.get('div.form-group__text > label').should('contain', i18n._WhatCanWeDoToImprove_);
			...
			cy.get('[class="checkbox__label hidden-xs text-small"]').should('contain', i18n._CiscoCanContactMe_);
