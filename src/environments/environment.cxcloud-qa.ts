...
export const environment = {
	...
	sdpServiceBasePath: (`${defaults.datacenterStub}` === 'usa') ? '/api' : `/api/${defaults.datacenterStub}`,
	...
	sdpServicePaths: {
		contracts: (`${defaults.datacenterStub}` === 'usa') ? '/api/v1/contracts' : `/api/${defaults.datacenterStub}/v1/contracts`,
		customerportal: (`${defaults.datacenterStub}` === 'usa') ? '/api/customerportal' : `/api/${defaults.datacenterStub}/customerportal`,
		cxportal: (`${defaults.datacenterStub}` === 'usa') ? '/api/cxportal' : `/api/${defaults.datacenterStub}/cxportal`,
		inventory: (`${defaults.datacenterStub}` === 'usa') ? '/api/v1/inventory' : `/api/${defaults.datacenterStub}/v1/inventory`,
		partner: (`${defaults.datacenterStub}` === 'usa') ? '/api/cxportal' : `/api/${defaults.datacenterStub}/cxportal`,
		cp: '/api/usa',
	},
