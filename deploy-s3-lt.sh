...
resetAws $1
# build the project
npx ngdeploy lt -vn --source='src' --gzip --no-push --no-validate --ng-build="--aot --optimization=true --baseHref /"
# sync dist files to s3
aws s3 sync dist s3://cisco-cx-customer-portal-lt
# invalidate the previous cache
invalidate=`aws cloudfront create-invalidation --distribution-id E2HK2RIKERL8A4 --paths '/*'`
...
while [[ $complete != *"Completed"* ]]; do
	complete=`aws cloudfront get-invalidation --distribution-id E2HK2RIKERL8A4 --id $id`
